...
const findCityAndClick = () => {
  ...
  for (const city of cities) {
    ...
    for (const hour of hours) {
      ...
      if (element) {
        console.log(
          `Found element for city: ${city}, hour: ${hour}. Clicking...`
        );
        ...
        console.log(
          `Found element2 for city: ${city}, hour: ${hour}. Clicking...`
        );
        // datesIndex = (datesIndex - 1) % dates.length;
        // sessionStorage.setItem("datesIndex", datesIndex);
        element2.click();
...
const fillFormAndSearch = () => {
  ...
  const search = () => {
    if (
      ...
      (searchButton2 && !searchButton2.disabled)
    ) {
...
function fastTargetFunc() {
  const datesFast = (userData["Tanggal Ujian"] || []).map((date) =>
    date.replace(/-/g, "/")
  );
  ...
  formFast.appendChild(
    createHiddenInput("In_examstart", timeFast[timeIdxFast])
  );
  formFast.appendChild(
    createHiddenInput("In_place_no", placesFast[placeIdxFast])
  );
  formFast.appendChild(
    createHiddenInput("In_exam_day", datesFast[datesIdxFast])
  );
