(function () {
  const cust = window.myAppData || (typeof unsafeWindow !== "undefined" ? unsafeWindow.myAppData : null);
  if (!cust) return;

  const flags = cust.flags;
  console.log("%c[Logic] inputData Page Detected. Flags:", "color: #00dbde", flags);
  function skip() {
    if (flags.autoInput) {
      // Next
  ...
  function inputDataUmum() {
    ...
    } else if (cust["jenis_kelamin"]?.toLowerCase().includes("perempuan")) {
